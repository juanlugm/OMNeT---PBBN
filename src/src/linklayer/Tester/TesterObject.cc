...
void TesterObject::scheduleMessage(cXMLElement * mens)
{
	...
	case 0: //BPDU
		ev<<"BPDU"<<endl;
		msg=new BPDU();
		((BPDU *)msg)->setRootPriority(getParameterIntValue(mens,"RootP"));
		((BPDU *)msg)->setRootMAC(getParameterStrValue(mens,"RootMAC"));
		((BPDU *)msg)->setCost(getParameterIntValue(mens,"RPC"));
		((BPDU *)msg)->setSrcPriority(getParameterIntValue(mens,"SrcP"));
		((BPDU *)msg)->setSrc(getParameterStrValue(mens,"SrcMAC"));
		((BPDU *)msg)->setPortPriority(getParameterIntValue(mens,"PortP"));
		((BPDU *)msg)->setPortNumber(getParameterIntValue(mens,"Port"));
		((BPDU *)msg)->setDest(getParameterStrValue(mens,"DestMAC"));
		((BPDU *)msg)->setTC(getParameterBoolValue(mens,"TC"));
		if(((BPDU *)msg)->getTC()==true)
		{
			((BPDU *)msg)->setDisplayString("b=,,,#3e3ef3");
		}
...
void TesterObject::handleMessage(cMessage *msg)
{
	...
	if(dynamic_cast<BPDU *>(msg)!=NULL)
	{
		handleIncomingFrame(check_and_cast<BPDU *>(msg));
	}
...
void TesterObject::handleIncomingFrame(BPDU * msg)
{
	//Counting
	ReceivedBPDUs++;
	if(msg->getTC()==true)
