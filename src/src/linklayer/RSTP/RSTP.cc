...
void RSTP::initialize(int stage)
{
	...
	if(stage==1) // "auto" MAC addresses assignment takes place in stage 0.
	{
		...
		if(macUnit!=NULL)
		{
			address.setAddress(check_and_cast<EtherMAC::EtherMAC *>(macUnit)->par("address"));
		}
		else
		{
		...
		pFile = fopen (buff,"w");
		  if (pFile!=NULL)
		  {
			...
			for(unsigned int i=0;i<Puertos.size();i++)
			{
				...
				else if (Puertos[i].PortRole==ALTERNATE)
...
void RSTP::handleHelloTime(cMessage * msg)
{
	...
	{ //Sends hello through all (active ports). ->learning, forwarding or not asigned ports.
		...
		if((Puertos[i].Edge!=true)&&((Puertos[i].PortRole==ROOT)||(Puertos[i].PortRole==ALTERNATE)||(Puertos[i].PortRole==BACKUP)))
		{
			Puertos[i].LostBPDU++;
			...
			{//Starts contest.
				...
				else if((Puertos[i].PortRole==ALTERNATE)||(Puertos[i].PortRole==BACKUP))
...
{  //Incoming BPDU handling
	...
	BPDU * frame=check_and_cast<BPDU *>(frame2->decapsulate());
	delete frame2;
		...
		else
		{
					...
					switch(caso2)
					{
						case 0: //Double link to the same port of the root source.Tie breaking. Better local port first.
									...
									||((Puertos[r].PortPriority==Puertos[arrival].PortPriority)&&(r<arrival)))
							{
								...
								Puertos[arrival].Flushed++;
								sw->flush(arrival);
								Puertos[arrival].PortRole=ALTERNATE;
								Puertos[arrival].PortState=DISCARDING;
								Puertos[arrival].LostBPDU=0;
							}
							else
						...
						case 4: //Same that Root RPC Source but better port
							...
							Puertos[arrival].PortRole=ROOT;
							Puertos[arrival].PortState=FORWARDING;
							Puertos[r].PortRole=ALTERNATE; //Temporary. Just one port can be root at contest time.
							Puertos[arrival].LostBPDU=0;
							sw->cpCache(r,arrival); //Copy cache from old to new root.
							Flood=true;
			...
			{//Source has updated BPDU information.
				...
				case -1://Worse Root
					...
					else if(Puertos[arrival].PortRole==ALTERNATE)
					{
						Puertos[arrival].PortState=DISCARDING;
						Puertos[arrival].PortRole=DESIGNATED;
						Puertos[arrival].updatePortVector(frame,arrival);
...
{//Send BPDUs through all ports if they are required.
	...
	for(unsigned int i=0;i<Puertos.size();i++)
	{
		if((Puertos[i].PortRole!=ROOT)&&(Puertos[i].PortRole!=ALTERNATE)&&(Puertos[i].PortRole!=DISABLED)&&(Puertos[i].Edge!=true))
		{
			sendBPDU(i);
		}
	}
...
{//Initializing PortRstpVector.
	...
	this->RootMAC=new MACAddress::MACAddress(address);
	this->RootPathCost=0;
	this->Age=0;
	this->srcPriority=priority;
	this->srcAddress=new MACAddress::MACAddress(address);
...
int RSTP::contestRstpVector(BPDU* msg,int arrival)
...
int RSTPVector::compareRstpVector(BPDU * msg,int PortCost)
...
{// Gets best alternate port index.
	...
	for(unsigned int j=0;j<Puertos.size();j++)
	{
		if(Puertos[j].PortRole==ALTERNATE) //Just from Alternates. Others are not updated.
		{
...
void PortStatus::updatePortVector(BPDU *frame,int arrival)
...
MACAddress::MACAddress RSTP::getAddress()
{
return address;
}
