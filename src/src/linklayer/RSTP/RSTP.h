...
public:
			  int RootPriority;
			  MACAddress::MACAddress * RootMAC;
			  int RootPathCost;
			  int Age;
			  int srcPriority;
			  MACAddress::MACAddress *srcAddress;
			  int srcPortPriority;
			  int srcPort;
			  int arrivalPort;
			   ...
			   */
			  virtual int compareRstpVector(BPDU * frame,int PortCost);
...
public:
	...
	PortStatus();
	virtual void updatePortVector(BPDU *frame,int arrival);
...
class RSTP: public cSimpleModule
{
  protected:
	 ...
	 Cache1Q * sw;  /// Needed for flushing.
	  ...
	  MACAddress::MACAddress address; /// BEB MAC address
  ...
  public:
     ...
     */
    virtual MACAddress::MACAddress getAddress();
  ...
  protected:
     ...
     * -4=Worse Port -3=Worse Src -2=Worse RPC -1=Worse   0= Similar  1=Better Root. 2= Better RPC  3= Better Src   4= Better Port
     */
    virtual int contestRstpVector(BPDU *frame, int arrival);
